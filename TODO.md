...
- [ ] **Address the audit B-3 through B-9 findings** — ⚠ TRIAGED 2026-07-05: **B-3 ✅ FIXED 2026-07-05** (dead `lib/wasm` fallback → package-root-aware `defaultWasmLocation()` in all 3 loaders; browser branch now bundle-relative; tarballs verified shipping the binary) **+ B-4 ✅ FIXED 2026-07-05** (fullMatrices implemented — option was accepted-but-ignored; both skips unskipped with orthonormality pins, SVD suite 0 skips) **+ B-5 (upstream-fix audit) remains**; B-7 ✅ (turbo 2.9.18, `npm audit` = 0 vulns), B-8 ✅ moot (the AssignmentNode FIXME no longer exists), B-9 ✅ moot (the Unit merge made `core/src/types/unit/Unit.ts` the LIVE implementation). B-3 has fresh evidence: the 2026-07-04 flake-hunt log caught expression falling back to JS on `ENOENT …\Github\lib\wasm\mathts-as.wasm` — the legacy `../../../lib/wasm` fallback path in `matrix/src/backends/WasmLoader.ts:701`. Open after the

...
>   - [x] **2a — assessment + pilot** (branch `bun-phase2a`, PR not merged): `trigonometry` and
>         `statistics` now run `bun test`. Same tests, same counts (1 -> 1, 6 -> 6); the full CI gate
>         is green. The `vitest` import specifier is kept: `bun test` resolves it to `bun:test`, so
...
>         not the repo root). - [ ] **`packages/workerpool` stays on vitest.** Its suites dispatch to real workers, and the
>         `workerpool` library never answers under Bun 1.4.2 (`workerType` `thread` and
>         `process` both hang; the same probe passes under Node). 20 tests timed out at 30 s
>         each and none passed. Revisit when Bun's `worker_threads` / `child_process` fixes
>         land, or when `workerpool` is replaced.
...
>         `__PKG_VERSION__` define (bunfig `[test] preload`) that the vitest configs now supply.

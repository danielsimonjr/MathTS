...
- Output for ordinary inputs is byte-identical: the 15 golden SVG snapshots did not change.
  Architecture numbers refreshed (`totalLinesOfCode` 337758); `repo_map.py check` passes.
...
- Updated with `main` by merge (after #281 and #282). Generated docs regenerated with `docs:deps` on a
  built WASM bundle; Verification tables now read 1903 files and 334757 lines (`repo_map.py map`).
...
- **Dependabot switched back from the `bun` ecosystem to `npm`.** The Bun migration (#274)
  pointed it at `bun`, but Dependabot's bun support parses only `bun.lock` lockfileVersion 1
  while Bun 1.4.2 writes 2, so the first run after the merge failed with
  `Unsupported bun.lock 'lockfileVersion' 2` and no dependency update could be proposed.
  Already diagnosed and fixed twice in this fleet (deepthinking-mcp `524ada7f`, fzf-mcp
  `387ee494`); Phase 1 re-introduced it here.
  Worth stating plainly because of how it hides: security **alerts** keep working (they are
  dependency-graph based), so nothing turns red — it is automated **remediation** that dies, and
  "0 open PRs" then reads as health when it partly means Dependabot cannot open one.
  `bun.lock` remains authoritative and is regenerated by `bun install`; the npm ecosystem updates
  `package.json` ranges only, so there is no second lockfile.

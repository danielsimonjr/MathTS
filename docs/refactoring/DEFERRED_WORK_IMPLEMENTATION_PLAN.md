...
Unit.prototype.simplify = function () {
  ...
  const simplifiedUnits = [...numerator, ...denominator.map((u) => ({ ...u, power: -u.power }))];
...
Unit.prototype._reconstructUnitString = function (units) {
  ...
  let str = numerator.map((u) => this._formatUnitPart(u)).join(' ');

  if (denominator.length > 0) {
    str +=
      ' / ' +
      denominator.map((u) => this._formatUnitPart({ ...u, power: Math.abs(u.power) })).join(' / ');
  }
...
describe('parseNumberWithConfig', function () {
  ...
  it('should throw error for non-string input', function () {
    assert.throws(() => math.parseNumberWithConfig(42), /TypeError.*expects string/);
  });
});
```
    ...
    return typed('prod', {
      ...
      Array: function (x) {
        ...
        const converted = x.map((element) =>
          typeof element === 'string' ? parseNumberWithConfig(element) : element
        );
...
_Part of Skipped Tests Resolution Effort_

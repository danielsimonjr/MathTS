...
class BackendManager {
  ...
  select(operation: string, elementCount: number): MatrixBackend {
    ...
    if (preferred === 'gpu' || (preferred === 'auto' && elementCount >= thresholds.gpuMinElements)) {
    ...
    if (preferred === 'wasm' || (preferred === 'auto' && elementCount >= thresholds.wasmMinElements)) {
  ...
  get js(): JSBackend { return this.jsBackend; }
  get wasm(): WASMBackend | null { return this.wasmBackend; }
  get gpu(): GPUBackend | null { return this.gpuBackend; }
}
...
export function transpose(
  inPtr: usize, rows: i32, cols: i32,
  outPtr: usize
): void;

export function elementwise_add(
  aPtr: usize, bPtr: usize, len: i32,
  outPtr: usize
): void;

export function lu_decompose(
  inPtr: usize, n: i32,
  lPtr: usize, uPtr: usize, pPtr: usize
): i32; // returns 0 on success, -1 on singular matrix
```
...
*This document serves as the canonical reference for MathTS development. Update as architecture evolves.*

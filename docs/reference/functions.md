...
| `factor(expr)`                                         | Factor; **univariate** integer polynomials factor **completely into irreducibles over ℤ/ℚ** via the Zassenhaus algorithm (`x^4-1` → `(x-1)*(x+1)*(x^2+1)`; ℚ-irreducibles like `x^4+1` returned unchanged); **multivariate** polynomials get integer-content / common-monomial / difference-of-squares extraction (e.g. `x^2*y+x*y^2` → `x*y*(x+y)`, `4*x^2-9*y^2` → `(2*x-3*y)*(2*x+3*y)`), everything else falls back to integer-GCD extraction (full multivariate factorization — Wang/EEZ — is future work) |
...
- `expand`, `factor`, `apart`, and `together` perform real transforms for the
  **univariate** case (a single variable): `factor` factors any
  ...
  unchanged; `expand` fully expands; `together` combines any sum of rational
  terms over a common denominator; `apart` decomposes a proper rational
  function whose denominator factors into **distinct** rational linear factors
  (repeated roots and irreducible higher-degree denominators are out of scope
  and pass through unchanged). Inputs outside scope (multiple variables,
  non-integer coefficients, function calls) fall back to each function's
  original implementation (`factor`'s integer-content / common-monomial /
  difference-of-squares extraction for multivariate, `together`/`apart`'s
  numeric-only fraction arithmetic). **Full multivariate factorization**
  (Wang/EEZ) and symbolic integration remain future work.

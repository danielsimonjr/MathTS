...
- **`WebGPU`** — the function runs on the GPU via WebGPU compute shaders for
  large inputs (32-bit float precision). **Browser-only**, and carried today by
  exactly four standalone helpers (see
  [WebGPU-accelerated operations](#webgpu-accelerated-operations)) — no
  typed-dispatch function has a WebGPU path yet.
...
> **WebGPU coverage is currently zero at the typed-dispatch layer.** The generated
> report [`docs/Architecture/webgpu-pairing.md`](../Architecture/webgpu-pairing.md)
> (data in `webgpu-pairing.json`) is the authoritative map; as of the last
> regeneration it reports **0 GPU-accelerated of 218 typed functions**. The only
> WebGPU code paths in this package are the four standalone `gpu*` helpers below,
> which are not typed-dispatch entries and so are not counted there. Wiring the
> typed layer to the GPU (a `*GpuDispatch` tier, behind an explicit opt-in) is a
> planned effort — see [`ROADMAP.md`](../../ROADMAP.md).
...
GPU compute-shader implementations of four core matrix operations. **These four
are the only WebGPU code paths in the library** — they are standalone helpers,
deliberately named `gpu*`, not typed-dispatch entries. Calling `multiply` or
`transpose` never reaches the GPU.

...
- **WASM acceleration:** `besselJ0` / `besselJ1` / `besselJ(n, x)`,
  ...
  `ellipticPi`, `carlsonRC/RD/RF/RJ`, and `lgamma` route to Rust+AssemblyScript
  WASM kernels for `Float64Array` inputs of length ≥ 1024
  (`WASM_SPECIAL_THRESHOLD`); the `functions` package bundles the Rust wasm and
  ...
  mpmath: `functions/tests/diff-special.test.mjs`, `tests/diff-wasm-rust.test.mjs`).

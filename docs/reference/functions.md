...
> **8 standalone functions** that do route to the GPU (`fuseUnaryChainAsync`,
> `fuseUnaryChainReduceAsync`, `elementwiseChainGpuDispatch`,
> `elementwiseChainReduceGpuDispatch`, and the four `gpu*` matrix helpers), and reports
...
> same economics that retired element-wise ops from the WASM backend. The GPU wins
> only where work is amortized over the transfer: a **fused chain** of ops
> (`fuseUnaryChainAsync`, 3.2–8.3× over WASM, measured) or a large **matmul**.
> Wiring every typed function to a GPU path would make the number look better and
> the library slower, so we don't.

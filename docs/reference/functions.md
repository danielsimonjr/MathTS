...
rule sets. MathTS implements the **rational-function** case of Risch completely:
a rational integrand whose denominator factors over ℚ into linear and
irreducible-quadratic factors is integrated in closed form (rational part via
partial fractions over the #7 polynomial-factorization engine, plus `log` and
`arctan` terms). The Rothstein–Trager algorithm (degree-≥3 irreducible and
positive-discriminant denominators) and the transcendental Risch tower (`exp`/`log`
extensions) are future layers. The integral transforms have deep roots: the Laplace transform comes

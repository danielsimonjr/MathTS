...
| `symbolicIntegral(expr[, variable])`       | Recursive symbolic antiderivative (power rule, linearity, `1/x`, linear-substitution of `sin`/`cos`/`exp`/`ln`/`sinh`/`cosh`), plus **partial-fraction integration** of rational functions (`1/(x^2-1)` → sum of logs) and **tabular integration by parts** for polynomial·{`exp`,`sin`,`cos`} (`x·sin(x)` → `sin(x)−x·cos(x)`); returns an unevaluated `integral(expr, variable)` marker when out of scope (non-linear inner arguments, general u-substitution/Risch, irreducible-quadratic denominators) |

...
> (`fuseUnaryChainAsync`, ~2–2.5× measured) or a large **matmul**. Wiring every
> typed function to a GPU path would make the number look better and the library
> slower, so we don't.
...
enableGpu(); // opt in — GPU results are f32, so this is never automatic
const ys = await fuseUnaryChainAsync(['sin', 'exp', 'tanh'], xs); // Float32Array
```
...
**WASM (f64) → GPU (f32) → JS (f64)** and returns a `Float32Array` only when the
GPU path ran (the return type _is_ the precision contract). It is a separate
`async` function rather than a change to the synchronous `fuseUnaryChain`,
because a GPU dispatch is inherently asynchronous.
...
### Honest measurement: for element-wise work, WASM beats the GPU

Chain `sin→exp→tanh→cos`, Chrome on an NVIDIA Pascal adapter:
...
**WASM is ~1.8× faster than the GPU, and it is f64-exact while the GPU is f32.**
For element-wise chains the GPU is therefore _both slower and less precise_, so
it is tried **after** WASM, never before. It still earns its place where WASM is
unavailable — there it beats the JS scalar pass ~2–2.5×.

This is why the GPU tier is opt-in and narrow: **the GPU wins on compute-bound
work (a large `gpuMatmul`, O(n³) over O(n²) transfer), not on memory-bound
element-wise work.** Opting in does not make element-wise code faster if WASM is
available — it is there for environments where WASM cannot load.

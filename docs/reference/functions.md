...
**`fuseUnaryChainAsync(ops, xs)`** — the fused element-wise chain, and the one
place the GPU is genuinely worth using for element-wise work. It tries **GPU
(f32) → WASM (f64) → JS (f64)** and returns a `Float32Array` only when the GPU
path ran (the return type _is_ the precision contract). It is a separate `async`
...
across the whole chain. (This is the same economics that retired element-wise
ops from the WASM backend.)

The GPU engages only when you have opted in, a device exists, the array clears
**65,536 elements**, and every op in the chain has a GPU kernel. Otherwise the
call transparently falls through to the CPU tiers. Measured on an NVIDIA Pascal
adapter for the chain `sin→exp→tanh→log1p`, against the browser's CPU path:

...
| `factor(expr)`                                         | Factor; univariate polynomials factor over ℚ via the rational-root theorem, multivariate polynomials get integer-content / common-monomial / difference-of-squares extraction (e.g. `x^2*y+x*y^2` → `x*y*(x+y)`, `4*x^2-9*y^2` → `(2*x-3*y)*(2*x+3*y)`), everything else falls back to integer-GCD extraction (full multivariate factorization is out of scope) |

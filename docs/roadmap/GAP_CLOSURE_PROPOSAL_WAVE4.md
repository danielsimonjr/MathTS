...
> **Status (2026-06-25, superseded):** delivered well past the ~1e-7 target. Bessel and Airy were rewritten off the NR §6 tradition onto ascending series + Hankel/DLMF-9.7 asymptotics (coefficients generated by recurrence) and are now validated to **<1e-9** vs mpmath in both the JS and WASM paths. See `docs/Architecture/WASM_ACCELERATION.md` and the `diff-special` / `diff-wasm-rust` tests.

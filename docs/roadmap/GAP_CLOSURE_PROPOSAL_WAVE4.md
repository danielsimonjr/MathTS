...
- `assembly/src/poly.ts` — add `poly_resultant_f64` and `poly_discriminant_f64`. Build the (m+n-2)×(m+n-2) Sylvester matrix, route through the existing det kernel (use the existing `det()` from `assembly/src/algebra/decomposition.ts` or inline if not exposed).
...
- `assembly/src/special.ts` — NEW. AssemblyScript WASM kernel for all 6 Bessel exports (`bessel_j0/j1/jn/y0/y1/yn_f64`) **and** `airy_ai_f64` / `airy_bi_f64`. Use series + asymptotic expansion per Numerical Recipes §6.7.
- `assembly/src/index.ts` — re-export.
- `functions/src/wasm/special/wasm-bridge.ts` — add `airyAiDispatch` / `airyBiDispatch` following the existing probe-WASM-then-JS pattern. The kernel probing is already wired (forward-compat from 3.10c-1).
- `functions/src/wasm/WasmLoader.ts` — register `airy_ai_f64`, `airy_bi_f64`, and the Bessel variants (`bessel_j0_f64` etc.).
...
> **Status (2026-06-25, superseded):** delivered well past the ~1e-7 target. Bessel and Airy were rewritten off the NR §6 tradition onto ascending series + Hankel/DLMF-9.7 asymptotics (coefficients generated by recurrence) and are now validated to **<1e-9** vs mpmath in both the JS and WASM paths. See `docs/Architecture/WASM_ACCELERATION.md` and the `diff-special` / `diff-wasm` tests.

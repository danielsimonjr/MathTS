...
| `typed/cas.ts` → `polyFit`, `chebyshevFit`, `legendreFit` (✅ `f537a56`)                                                                                         | Normal-equations or QR over big sample arrays             | Vandermonde fill + QR/normal-eqs solve                         | `wasm.poly_fit_f64` + `cheb_fit_f64` + `legendre_fit_f64` (inlined Householder QR ~230 LOC Rust + ~170 LOC AS + ~170 LOC bridge)                                            | ≥ 1024 samples          | Wave-5 Slice 5.4 — inlined Householder QR rather than calling into the matrix-package qr to avoid cross-crate coupling.                                                                                                                                       |
...
- `besselJ` / `besselY` / `airyAi` / `airyBi` / `ellipticK` / `ellipticE` — `572363f` + `276a75b` + `098656e`.
- `goertzel` / `chirpZTransform` / `welchPSD` / `bartlettPSD` / `multiTaperPSD` — `2d0ebfa` (Slice 5.6).
- `convexHull` (2-D Andrew's monotone-chain) — `5a0ca7c` (Slice 5.7d). 3-D hull + Delaunay deferred (need orient2D/inCircle predicate kernels).
- `pinv` / `expm` / `logm` / `sqrtm` matrix-function evaluators — `0cef320` + `ca08c12` (Slices 5.2 + 5.9a).
...
Only three items remain forward-tracked:

- **WebGPU browser smoke test** — infra prerequisite (Playwright PR + CI matrix entry on a runner with software WebGPU adapter). Not an implementation slice.
- **Slice 5.9b** — full Higham general-case `logm` / `sqrtm` for non-diagonalisable / complex-eigenvalue matrices, plus Schur-based Björck-Hammarling sqrtm. Slice 5.9a covers the SPD / real-positive-eigenvalue case.
- **Incomplete elliptic integrals + Carlson symmetric forms** (`ellipticF`, `ellipticPi`, `RC` / `RD` / `RF` / `RJ`) — deferred from Slice 5.3 as a future sibling.

No new gaps. The gap-closure work the audit set out to do is complete.

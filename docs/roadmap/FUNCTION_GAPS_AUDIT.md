...
- **`typed/algebra.ts` WASM routing for polynomial ops** — ✅ landed in commit `6520a76` (GAP_CLOSURE Slice 3.7). Two kernels in `wasm-rust/crates/mathts-wasm/src/poly.rs` (`poly_mul_f64`, `poly_div_mod_f64`) with AS parity in `assembly/src/poly.ts`; bridge in `functions/src/wasm/poly/wasm-bridge.ts` at `WASM_POLY_THRESHOLD = 256` coeffs; wires into `polymul`, `polynomialGCD`, `polynomialLCM`, `polynomialQuotient`, `polynomialRemainder`. 22 new tests. (`discriminant`, `resultant` deferred — they'll reuse the new `poly_div_mod_f64` plus a Sylvester-fill helper in a follow-up slice.)
...
| `typed/special.ts` → `besselJ`, `besselY` (✅ `572363f`); `airyAi`, `airyBi`, `ellipticK`, `ellipticE` deferred | Per-element series / continued fraction                   | Recurrence + checks                                            | `wasm.bessel_j0_f64` + family (Rust only, no AS yet)       | ≥ 1024 samples          | Bessel J/Y landed: scalar NR §6.5 implementations already existed in `special/functions.rs`; array kernels delegate to scalars. Airy + AS parity = Slice 3.10c-2. |

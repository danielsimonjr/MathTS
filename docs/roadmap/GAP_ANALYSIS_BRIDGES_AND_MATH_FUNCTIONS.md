...
The `functions` package exposes three layers — ~320 native "typed" functions,
161 newly-activated mathjs factory functions, and 72 factory functions shadowed
by typed equivalents. The April roadmap's Phase 1–3 backlog (CAS, graph theory,
distributions, hypothesis tests, special functions, numerical methods) has been
substantially delivered as native TypeScript in `functions/src/typed/`.
...
| Layer | Count | Notes |
|-------|------:|-------|
| Active typed functions | ~320 | `functions/src/typed/` (17 modules) |
...
- Cross-referenced against `factoriesAny.ts` (372 factory refs) and
  `factoriesNumber.ts` (225 refs) for full mathjs API coverage.
...
`functions/src/typed/index.ts` and from the package root.

| Module | Domain | Approx. public fns |
|--------|--------|-------------------:|
| `arithmetic.ts` | add/sub/mul/div, powers, logs, rounding, gcd/lcm, norm | ~48 |
| `trigonometry.ts` | sin/cos/tan + inverses + hyperbolic, hypot, deg/rad | ~20 |
| `combinatorics.ts` | fibonacci/lucas, factorials, primes, number theory | ~21 |
| `statistics.ts` | parallel-first sum/mean/var/std/median/mode/corr/MAD | ~17 |
| `signal.ts` | FFT2D, DCT/DST, DWT, filters, convolution/correlation | ~31 |
| `special.ts` | Bessel, beta/gamma, digamma, elliptic, erf, Fresnel | ~29 |
| `distributions.ts` | normal/exp/bernoulli/binomial/poisson/geometric PDF/CDF/PMF | ~11 |
| `dist-objects.ts` | 12 distribution objects (`.pdf`/`.cdf`/`.quantile`) | 12 |
| `algebra.ts` | polynomial ops, expand/factor/collect/substitute, trig CAS | ~37 |
...
| `geometry.ts` | distances, areas, hulls, Delaunay/Voronoi, kd-tree | ~28 |
...
Comparing the activated + typed surface against `factoriesAny.ts`:
...
CAS (~28), special functions (~29), numerical methods (~34), extended
combinatorics/number theory. That roadmap should be marked **largely
...
| 9 | Stateful `Parser` object + JSON `reviver`/`replacer` | 3.3 | M | **P3** |

...
| **N7** | **`ThresholdDispatcher` is orphaned.** A category-based `ThresholdDispatcher` (`parallel/src/strategies/threshold.ts:86`) exists but is **not** wired into typed functions; the typed layer uses a *separate* `ComputePool.shouldParallelize` path. ✅ **Consolidated (GC14):** ComputePool's benchmark-tuned `DEFAULT_THRESHOLD_BY_OP` is now the single exported source of truth; `ThresholdDispatcher` derives its overlapping `matmul` value from it (was 10000, now the canonical 4096), with a guard test pinning agreement. Also corrected a stale claim — `fft` **does** auto-dispatch to the WASM tier by size (`functions/src/matrix/fft.ts:279-284`, `length ≥ WASM_FFT_THRESHOLD && isPowerOf2`); `parallelFFT` is a separate explicit *worker* tier, so the "parallel-only-named" framing was imprecise. | done (GC14) | — |

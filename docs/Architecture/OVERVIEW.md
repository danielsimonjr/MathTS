...
| Claim                | Value  | Source                                                                                                                                |
| -------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------- |
| totalTypeScriptFiles | 1907   | `repo_map.py map . --out <tmp>` (skills/architecture-docs), metadata of its dependency-graph.json; `repo_map.py check` fails on drift |
| totalLinesOfCode     | 334891 | `repo_map.py map . --out <tmp>` (skills/architecture-docs), metadata of its dependency-graph.json; `repo_map.py check` fails on drift |
| totalExports         | 7655   | `repo_map.py map . --out <tmp>` (skills/architecture-docs), metadata of its dependency-graph.json; `repo_map.py check` fails on drift |

...
| Claim                | Value  | Source                                                                                                                                |
| -------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------- |
| totalTypeScriptFiles | 1908   | `repo_map.py map . --out <tmp>` (skills/architecture-docs), metadata of its dependency-graph.json; `repo_map.py check` fails on drift |
| totalLinesOfCode     | 334965 | `repo_map.py map . --out <tmp>` (skills/architecture-docs), metadata of its dependency-graph.json; `repo_map.py check` fails on drift |
| totalExports         | 7657   | `repo_map.py map . --out <tmp>` (skills/architecture-docs), metadata of its dependency-graph.json; `repo_map.py check` fails on drift |

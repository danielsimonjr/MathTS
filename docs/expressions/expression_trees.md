...
const doubled = node.transform((n: Node) => {
  if (n.isConstantNode && typeof n.value === 'number') {
    return new ConstantNode(n.value * 2);  // replace
  }
  return n;  // keep
});
```
...
Nodes from `parse()` can be evaluated directly using the `compile` API from the
expression package:

```typescript
import { parse } from '@danielsimonjr/mathts-functions';
import { compile } from '@danielsimonjr/mathts-expression';
import { factoryScope } from '@danielsimonjr/mathts-functions/factories/scope';

const node = parse('x^2 + 1');
const compiled = compile(node, factoryScope);
compiled.evaluate({ x: 3 });   // 10
```

For most use cases, `evaluate()` and `compileExpr()` (see [parsing.md](parsing.md)) are
simpler — they handle compilation automatically.
...
Node constructors are exported from `@danielsimonjr/mathts-expression`. You rarely need
these directly — use `parse()` instead — but they are useful for programmatic tree
construction:
...
// Build sqrt(2 + x) manually
// (requires bootstrapped node constructors from the expression package)
const two  = new ConstantNode(2);
const x    = new SymbolNode('x');
const sum  = new OperatorNode('+', 'add', [two, x]);
const root = new FunctionNode('sqrt', [sum]);

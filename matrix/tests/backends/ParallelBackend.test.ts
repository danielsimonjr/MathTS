...
import { ParallelBackend, createParallelBackend, parallelBackend } from '../../src/backends/ParallelBackend.js';
...
describe('ParallelBackend', () => {
  ...
  describe('element-wise operations (parallel)', () => {
    it('should add matrices', async () => {
      const a = DenseMatrix.fromArray([[1, 2], [3, 4]]);
      const b = DenseMatrix.fromArray([[5, 6], [7, 8]]);
      const c = await backend.add(a, b);
      expect(c.toArray()).toEqual([[6, 8], [10, 12]]);
    });

    it('should subtract matrices', async () => {
      const a = DenseMatrix.fromArray([[5, 6], [7, 8]]);
      const b = DenseMatrix.fromArray([[1, 2], [3, 4]]);

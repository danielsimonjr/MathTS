 ...
 */
function mockTyped(_name: string, signatures: Record<string, Function>) {
...
describe('FunctionAssignmentNode - construction & identity', () => {
  ...
  it('has correct type property', () => {
    expect(new FunctionAssignmentNode('f', ['x'], makeConst(1)).type).toBe('FunctionAssignmentNode')
  })

  it('has isFunctionAssignmentNode = true', () => {
    expect(new FunctionAssignmentNode('f', ['x'], makeConst(1)).isFunctionAssignmentNode).toBe(true)
  })

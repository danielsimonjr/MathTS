...
// solveODE is a typed function (its call returns `unknown`); these are its result shapes here.
interface Solution {
  y: unknown[];
}

const last = (sol: unknown): number | number[] => {
  const { y } = sol as { y: number[] | number[][] };
  return y[y.length - 1];
};
...
describe('solveODE — scalar ODEs (JS reference path)', () => {
  it("y' = -y, y(0)=1  →  y(1) = e^-1", () => {
    const y1 = last(
      solveODE((_t: unknown, y: unknown) => -(y as number), [0, 1], 1, { tol: 1e-9 })
    ) as number;
  ...
  it("logistic y' = y(1-y), y(0)=0.5  →  y(t) = 1/(1+e^-t)", () => {
    const y2 = last(
      solveODE((_t: unknown, y: unknown) => (y as number) * (1 - (y as number)), [0, 2], 0.5, {
        tol: 1e-9,
      })
    ) as number;
  ...
  it("backward integration: from t=1 (y=e) back to t=0 gives e² for y'=-y", () => {
    ...
    const y0 = last(
      solveODE((_t: unknown, y: unknown) => -(y as number), [1, 0], Math.E, { tol: 1e-9 })
    ) as number;
  ...
  it('RK23 method also solves the scalar decay', () => {
    const y1 = last(
      solveODE((_t: unknown, y: unknown) => -(y as number), [0, 1], 1, {
        method: 'RK23',
        tol: 1e-9,
      })
    ) as number;
...
describe('solveODE — systems', () => {
  it('harmonic oscillator [y1, -y0], y(0)=[1,0]  →  [cos t, -sin t]', () => {
    const y = last(
      solveODE(
        (_t: unknown, s: unknown) => [(s as number[])[1], -(s as number[])[0]],
        [0, 1],
        [1, 0],
        { tol: 1e-9 }
      )
...
describe('solveODE — Rosenbrock stiff method (ode23s)', () => {
  it("solves a linear stiff system y' = diag(-1,-1000) y to its exact solution", () => {
    ...
    const sol = solveODE(
      (_t: unknown, y: unknown) => [-(y as number[])[0], -1000 * (y as number[])[1]],
      [0, 1],
      [1, 1],
  ...
  it('agrees with the explicit RK45 on a non-stiff problem', () => {
    const stiff = solveODE((_t: unknown, y: unknown) => -(y as number), [0, 1], 1, {
      method: 'Rosenbrock',
      tol: 1e-8,
    }) as Solution;
...
describe('solveODE — large stiff system (matrix-LU-routed per-step solve)', () => {
  ...
  it('RODAS (n=12) matches the exact heat-equation mode', () => {
    const sol = solveODE(f, [0, tEnd], y0, { method: 'RODAS', tol: 1e-8 }) as Solution;
    const y = sol.y[sol.y.length - 1] as number[];
  ...
  it('Rosenbrock (n=12) matches the exact heat-equation mode', () => {
    const sol = solveODE(f, [0, tEnd], y0, { method: 'Rosenbrock', tol: 1e-8 }) as Solution;
    const y = sol.y[sol.y.length - 1] as number[];
...
describe('solveODE — Unit-valued state (numeric time)', () => {
  it("y' = y with y0 = 5 m gives 5·e m (state-is-Unit falls to the whole-interval step, not NaN)", () => {
    ...
    const sol = solveODE((_t: unknown, y: unknown) => y, [0, 1], unit('5 m'), {
      tol: 1e-6,
    }) as Solution;

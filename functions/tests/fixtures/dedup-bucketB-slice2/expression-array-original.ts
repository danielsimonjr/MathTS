/**
 * Frozen snapshot of `expression/src/utils/array.ts` as it stood at the commit
 * BEFORE the Bucket B slice 2 redirect (`git show HEAD:expression/src/utils/array.ts`),
 * kept purely so `functions/tests/dedup-bucketB-equivalence.test.ts` can prove the
 * pre-redirect implementation is equivalent to the new core canonical. Only the
 * import paths below were adjusted (fixture lives outside the expression package);
 * the function bodies are untouched. Do not import this file from `src/`.
 */
import { isInteger } from '../../../../expression/src/utils/number.js';
import {
  isNumber,
  isBigNumber,
  isArray,
  ...
  Matrix,
  IndexDimension,
} from '../../../../expression/src/utils/is.js';
import { format } from '../../../../expression/src/utils/string.js';
...
import { deepStrictEqual } from '../../../../expression/src/utils/object.js';
...
function _validate<T>(array: T[], size: number[], dim: number): void {
  ...
  if (dim < size.length - 1) {
    ...
    for (i = 0; i < len; i++) {
      ...
      _validate(array[i] as unknown[], size, dimNext);
    }
  } else {

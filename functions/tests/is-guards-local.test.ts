 ...
 * `@danielsimonjr/mathts-core`. They are called in hot numerical loops (the studentized-
 * range / Tukey Simpson integration), where V8 inlines local guards but will NOT inline
 * them across a module boundary; a cross-module re-export measured ~40% slower on that
 * path (tipped `gap-tukey.test.ts` past its timeout). See the PERF banner in `is.ts`.
 *
 * This is a STRUCTURAL check, not a timing benchmark — deterministic, not flaky. It fails
 * the moment someone turns `is.ts` into `export * from '@danielsimonjr/mathts-core/...'`.
 */

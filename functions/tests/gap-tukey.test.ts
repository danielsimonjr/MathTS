 ...
 */
// Each nested-Simpson CDF/quantile solve is legitimately expensive (240 inner / 120 outer
// nodes, calibrated for scipy parity; quantile adds a bisection loop). They fit well under
// 5s in isolation but can exceed it under full-suite parallel load, so give them explicit
// headroom over the 5s default. This is a compute-cost accommodation, not masking — the
// numeric assertions below still verify correctness to the scipy oracle.
const SLOW_MS = 30000;

describe('gap — studentized range distribution', () => {
  it(
    'CDF / quantile match scipy.stats.studentized_range',
    () => {

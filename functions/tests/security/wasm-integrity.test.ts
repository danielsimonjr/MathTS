...
import { sha384OfBuffer, verifyWasmIntegrity, loadWasmManifest } from '../../src/wasm/integrity.js';

/**
 * Integrity test for the WASM SHA-384 manifest mechanism.
 *
 * We construct a temp dir with:
...
describe('wasm integrity (SHA-384 manifest)', () => {
  beforeAll(async () => {
    dir = mkdtempSync(join(tmpdir(), 'mathts-wasm-int-'));
  ...
  it('soft-warns and continues when no manifest is present', async () => {
    // wasm path in a directory without a manifest file
    const noManifestDir = mkdtempSync(join(tmpdir(), 'mathts-wasm-noman-'));
    try {
      const p = join(noManifestDir, 'orphan.wasm');
      writeFileSync(p, original);
      // Should resolve, not throw, since manifest is optional
  ...
  it('fails closed when {required: true} and manifest is missing', async () => {
    const noManifestDir = mkdtempSync(join(tmpdir(), 'mathts-wasm-req-'));
    try {
      const p = join(noManifestDir, 'orphan.wasm');
      writeFileSync(p, original);

...
export const createPinv = /* #__PURE__ */ factory(
  ...
  ({
            ...
            if (rows === cols) {
              try {
                ...
                if (
                  ...
                  err.message.match(/Cannot calculate inverse, determinant is zero/)
                ) {
                  // Expected
                } else {
                  throw err;
     ...
     * @private
     */
    function _rankFact(mat: Scalar[][], rows: number, cols: number): RankFactResult {
      const rref = _rref(mat, rows, cols);
      const C = mat.map((row) => row.filter((_, j) => j < rows && !_isZero(dot(rref[j], rref[j]))));

...
export const createMultiply = /* #__PURE__ */ factory(
  ...
  ({
     ...
     */
    function _validateMatrixDimensions(
      size1: ArraySize,
      size2: ArraySize
    ): void {
     ...
     * @returns Dense Vector (N)
     */
    function _multiplyVectorDenseMatrix(
      a: Matrix,
      b: DenseMatrix
    ): DenseMatrix {
      // a dense
      ...
      return (a as DenseMatrix).createDenseMatrix({
        ...
        size: [bcolumns],
        datatype:
          adt === (a as DenseMatrix)._datatype && bdt === b._datatype
            ? dt
            : undefined
      })
    }
     ...
     * @returns Dense Vector (M)
     */
    function _multiplyDenseMatrixVector(
      a: DenseMatrix,
      b: Matrix
    ): DenseMatrix {
      // a dense
      ...
      return a.createDenseMatrix({
        ...
        size: [arows],
        datatype:
          adt === a._datatype && bdt === (b as DenseMatrix)._datatype
            ? dt
            : undefined
      })
    }
     ...
     */
    function _multiplyDenseMatrixDenseMatrix(
      a: DenseMatrix,
      b: DenseMatrix
    ): DenseMatrix {
      // a dense
     ...
     */
    function _multiplyDenseMatrixSparseMatrix(
      a: DenseMatrix,
      b: SparseMatrix
    ): SparseMatrix {
      // a dense
     ...
     */
    function _multiplySparseMatrixVector(
      a: SparseMatrix,
      b: Matrix
    ): SparseMatrix {
      // a sparse
      ...
      for (let ib = 0; ib < brows; ib++) {
        ...
        if (!eq(vbi, zero)) {
          ...
          for (
            let ka0 = aptr![ib], ka1 = aptr![ib + 1], ka = ka0;
            ka < ka1;
            ka++
          ) {
            // a row
     ...
     * @returns SparseMatrix (MxC)
     */
    function _multiplySparseMatrixDenseMatrix(
      a: SparseMatrix,
      b: DenseMatrix
    ): SparseMatrix {
      // a sparse
     ...
     * @returns SparseMatrix (MxC)
     */
    function _multiplySparseMatrixSparseMatrix(
      a: SparseMatrix,
      b: SparseMatrix
    ): SparseMatrix {
      // a sparse
      ...
      for (let jb = 0; jb < bcolumns; jb++) {
        ...
        for (kb0 = bptr![jb], kb1 = bptr![jb + 1], kb = kb0; kb < kb1; kb++) {
          ...
          if (values) {
            // loop values in a[:,ib]
            for (
              ka0 = aptr![ib], ka1 = aptr![ib + 1], ka = ka0;
              ka < ka1;
              ka++
            ) {
              // row
    ...
    return typed(name, multiplyScalar, {
      ...
      'Array, Array': typed.referTo('Matrix, Matrix', ((
      ...
      'Matrix, Array': typed.referTo(
        'Matrix,Matrix',
        ((selfMM: TypedFunction) => (x: Matrix, y: any[]) =>
          selfMM(x, matrix(y))) as any
      ),

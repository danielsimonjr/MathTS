  ...
  ({
    ...
    function Unit(this: any, value?: any, valuelessUnit?: any) {
      ...
      if (valuelessUnit === undefined) {
        this.units = []
        this.dimensions = BASE_DIMENSIONS.map((_x: any) => 0)
        ...
        this.skipAutomaticSimplification =
          valuelessUnit.skipAutomaticSimplification
    ...
    Unit.prototype._isDerived = function () {
      ...
      return (
        this.units.length > 1 || Math.abs(this.units[0].power - 1.0) > 1e-15
      )
    }
     ...
     * @private
     */
    Unit.prototype._denormalize = function (value: any, _prefixValue: any) {
      if (value === null || value === undefined || this.units.length === 0) {
        return value
      }
      let res = value
    ...
    Unit.prototype.hasBase = function (base: any): boolean {
      ...
      for (let i = 0; i < BASE_DIMENSIONS.length; i++) {
        if (
          Math.abs((this.dimensions[i] || 0) - (base.dimensions[i] || 0)) >
          1e-12
        ) {
          return false
        }
    ...
    Unit.prototype.equalBase = function (other: any): boolean {
      ...
      for (let i = 0; i < BASE_DIMENSIONS.length; i++) {
        if (
          Math.abs((this.dimensions[i] || 0) - (other.dimensions[i] || 0)) >
          1e-12
        ) {
          return false
        }
    ...
    Unit.prototype.multiply = function (_other: any) {
      ...
      for (let i = 0; i < BASE_DIMENSIONS.length; i++) {
        ...
        res.dimensions[i] =
          (this.dimensions[i] || 0) + (other.dimensions[i] || 0)
      }
      ...
      if (this.value !== null || other.value !== null) {
        const valThis =
          this.value === null ? this._normalize(one(other.value)) : this.value
        const valOther =
          other.value === null ? other._normalize(one(this.value)) : other.value
    ...
    function getNumericIfUnitless(unit: any): any {
      if (
        unit.equalBase(BASE_UNITS.NONE) &&
        unit.value !== null &&
        !config.predictable
      ) {
        return unit.value
     ...
     * @private
     */
    function normalizeUnitKey(unitObj: any): string {
     ...
     * @private
     */
    function simplifyUnit(unit: any): any {
    ...
    Unit.prototype.abs = function () {
      ...
      for (const i in ret.units) {
        if (
          ret.units[i].unit.name === 'VA' ||
          ret.units[i].unit.name === 'VAR'
        ) {
    ...
    Unit.fromJSON = function (json: any) {
      ...
      unit.skipAutomaticSimplification =
        json.skipSimp ?? skipAutomaticSimplificationDefault
      return unit
    }
    ...
    function formatBest(unit: any, options: any = {}): any {
      ...
      const simp =
        unit.skipAutomaticSimplification || unit.value === null
          ? unit.clone()
          : unit.simplify()
    ...
    function applyBestPrefixIfNeeded(simp: any, offset: any): void {
      if (simp.units.length === 1 && !simp.fixPrefix) {
        ...
        if (
          Math.abs(simp.units[0].power - Math.round(simp.units[0].power)) <
          1e-14
        ) {
    ...
    Unit.prototype._bestPrefix = function (offset = 1.2) {
      ...
      let bestDiff =
        Math.log(absValue / Math.pow(bestPrefix.value * absUnitValue, power)) /
          Math.LN10 -
        offset
        ...
        for (const i in BASE_UNITS) {
          if (hasOwnProperty(BASE_UNITS, i)) {
            ...
            for (let j = 0; j < BASE_DIMENSIONS.length; j++) {
              if (
                Math.abs(
                  (newUnit.dimensions[j] || 0) -
                    (BASE_UNITS[i].dimensions[j] || 0)
                ) > 1e-12
              ) {
                match = false
                break

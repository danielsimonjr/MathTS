...
interface Index {
  ...
  dimension(
    dim: number
  ): number | { forEach(callback: (value: number, index: [number]) => void): void };
...
export const createSparseMatrixClass = /* #__PURE__ */ factory(
  ...
  ({ typed, equalScalar, Matrix: _Matrix }: SparseMatrixDependencies) => {
    ...
    class SparseMatrix extends _Matrix implements Matrix {
      ...
      constructor(data?: MatrixArray | Matrix | SparseMatrixConstructorData, datatype?: DataType) {
        super();
      ...
      subset(
        index: Index,
        replacement?: MatrixArray | Matrix | MatrixValue,
        defaultValue?: MatrixValue
      ): SparseMatrix | MatrixValue {
      ...
      reshape(sizes: number[], copy?: boolean): SparseMatrix {
        ...
        sizes.forEach(function (value) {
          if (!isNumber(value) || !isInteger(value) || value <= -2 || value === 0) {
            throw new TypeError(
              'Invalid size, must contain positive integers or -1 ' +
                '(size: ' +
                format(sizes, {}) +
                ')'
            );
       ...
       */
      map(
        callback: (value: MatrixValue, index: number[], matrix: SparseMatrix) => MatrixValue,
        skipZeros?: boolean
      ): SparseMatrix {
        // check it is a pattern matrix
        if (!this._values) {
       ...
       */
      forEach(
        callback: (value: MatrixValue, index: number[], matrix: SparseMatrix) => void,
        skipZeros?: boolean
      ): void {
        // check it is a pattern matrix
        if (!this._values) {
        ...
        for (let j = 0; j < columns; j++) {
          ...
          for (let k = k0; k < k1; k++) {
            ...
            str +=
              ...
              format(j, options) +
              ') ==> ' +
              (this._values ? format(this._values[k], options) : 'X');
          }
        }
        return str;
      }
    ...
    function _getsubset(matrix: SparseMatrix, idx: Index): SparseMatrix | MatrixValue {
      ...
      if (Number.isInteger(rows)) rowsCallback(rows as number, [0]);
        ...
        (rows as { forEach(callback: (i: number, r: [number]) => void): void }).forEach(
          rowsCallback
        );

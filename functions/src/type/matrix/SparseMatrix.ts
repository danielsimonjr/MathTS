  ...
  ({ typed, equalScalar, Matrix: _Matrix }: SparseMatrixDependencies) => {
    ...
    class SparseMatrix extends _Matrix implements Matrix {
      ...
      density(): number {
        ...
        return rows !== 0 && columns !== 0
          ? this._index.length / (rows * columns)
          : 0
      }
      ...
      *[Symbol.iterator](): Iterator<{ value: MatrixValue; index: number[] }> {
      ...
      static diagonal(
        ...
        if (isString(datatype)) {
          // find signature that matches (datatype, datatype)
          eq =
            (typed.find(equalScalar, [datatype, datatype]) as any) ||
            equalScalar
          // convert 0 to the same datatype
    ...
    function _createFromArray(
      ...
      if (isString(datatype)) {
        // find signature that matches (datatype, datatype)
        eq =
          (typed.find(equalScalar, [datatype, datatype]) as any) || equalScalar
        // convert 0 to the same datatype
        zero = typed.convert(0, datatype)
      }

 ...
 */
export function promoteLogarithm<C>(
  log16: number,
  numberLog: (x: number) => number,
  config: { predictable?: boolean },
  cplx: (x: number) => C
) {
  return function (b: bigint): number | C {
    ...
    return cplx((b as unknown as { toNumber(): number }).toNumber());
  };
}

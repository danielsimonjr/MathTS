 ...
 */
export async function sha384OfBuffer(
...
export async function verifyWasmIntegrity(
  ...
  const manifest =
    options.manifest !== undefined
      ? options.manifest
      : await loadWasmManifest(wasmPath)
  ...
  if (!expected) {
    if (options.required) {
      throw new Error(
        `WASM integrity check failed: manifest has no entry for "${fileName}"`
      )
    }
    return
  }
  ...
  if (actual !== expected) {
    throw new Error(
      `WASM integrity check failed for "${fileName}": ` +

 ...
 * @returns Interpolated y value
 */
export function linearInterpTable(xValuesPtr: usize, yValuesPtr: usize, x: f64, n: i32): f64 {
  ...
  if (x <= x0) {
    return linearInterp(x0, load<f64>(yValuesPtr), x1, load<f64>(yValuesPtr + 8), x);
  }
  if (x >= xn1) {
    return linearInterp(
      xn2,
 ...
 * @returns Interpolated y value
 */
export function lagrangeInterp(xValuesPtr: usize, yValuesPtr: usize, x: f64, n: i32): f64 {
  let result: f64 = 0.0;
 ...
 * @returns Interpolated y value
 */
export function newtonInterp(xValuesPtr: usize, coeffsPtr: usize, x: f64, n: i32): f64 {
 ...
 */
export function barycentricWeights(xValuesPtr: usize, n: i32, weightsPtr: usize): void {
  for (let i: i32 = 0; i < n; i++) {
    let w: f64 = 1.0;
    const xi = load<f64>(xValuesPtr + ((<usize>i) << 3));
    for (let j: i32 = 0; j < n; j++) {
...
export function clampedCubicSplineCoeffs(
  ...
  for (let i: i32 = 1; i < segments; i++) {
    ...
    const yip1 = load<f64>(yValuesPtr + ((<usize>(i + 1)) << 3));
    const yim1 = load<f64>(yValuesPtr + ((<usize>(i - 1)) << 3));
    store<f64>(alphaPtr + ((<usize>i) << 3), (3.0 / hi) * (yip1 - yi) - (3.0 / him1) * (yi - yim1));
  }
 ...
 * @returns Interpolated y value
 */
export function cubicSplineEval(xValuesPtr: usize, coeffsPtr: usize, x: f64, n: i32): f64 {
  const segments = n - 1;
 ...
 */
export function cubicSplineDerivative(xValuesPtr: usize, coeffsPtr: usize, x: f64, n: i32): f64 {
  const segments = n - 1;
 ...
 * @returns Interpolated value
 */
export function hermiteInterp(x0: f64, y0: f64, dy0: f64, x1: f64, y1: f64, dy1: f64, x: f64): f64 {
  const h = x1 - x0;
  if (h === 0) return y0;
...
export function polyFit(
  ...
  for (let k: i32 = 0; k < m; k++) {
    ...
    for (let i = k + 1; i < m; i++) {
      ...
      for (let j = k + 1; j < m; j++) {
        ...
        const val =
          load<f64>(ATAPtr + idx) - factor * load<f64>(ATAPtr + ((<usize>(k * m + j)) << 3));
        store<f64>(ATAPtr + idx, val);
      }
      const atyi =
        load<f64>(ATyPtr + ((<usize>i) << 3)) - factor * load<f64>(ATyPtr + ((<usize>k) << 3));
  ...
  for (let i = m - 1; i >= 0; i--) {
    ...
    for (let j = i + 1; j < m; j++) {
      sum -=
        load<f64>(ATAPtr + ((<usize>(i * m + j)) << 3)) * load<f64>(coeffsPtr + ((<usize>j) << 3));
    }

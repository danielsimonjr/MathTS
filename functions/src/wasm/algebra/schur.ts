  ...
  while (p > 0) {
    ...
    if (!deflated) {
      ...
      for (let iter: i32 = 0; iter < maxIter && !converged; iter++) {
        ...
        let z: f64 =
          l + 2 <= p ? load<f64>(TPtr + ((<usize>((l + 2) * n + (l + 1))) << 3)) * TLp1l : 0.0;
        ...
        for (let k: i32 = l; k < p; k++) {
          ...
          if (k < p - 1) {
            ...
            z = k + 3 <= p ? load<f64>(TPtr + ((<usize>((k + 3) * n + k)) << 3)) : 0.0;
          }
        }
  ...
  for (let k: i32 = 0; k < n - 2; k++) {
    ...
    for (let j: i32 = k; j < n; j++) {
      ...
      for (let i: i32 = 0; i < n - k - 1; i++) {
        ...
        store<f64>(APtr + idx, load<f64>(APtr + idx) - dot * load<f64>(vPtr + ((<usize>i) << 3)));
      }
    }
    ...
    for (let i: i32 = 0; i < n; i++) {
      ...
      for (let j: i32 = 0; j < n - k - 1; j++) {
        ...
        store<f64>(APtr + idx, load<f64>(APtr + idx) - dot * load<f64>(vPtr + ((<usize>j) << 3)));
      }
    }
    ...
    for (let i: i32 = 0; i < n; i++) {
      ...
      for (let j: i32 = 0; j < n - k - 1; j++) {
        const idx = (<usize>(i * n + (k + 1 + j))) << 3;
        store<f64>(QPtr + idx, load<f64>(QPtr + idx) - dot * load<f64>(vPtr + ((<usize>j) << 3)));
      }
    }
  }
}
 ...
 */
export function schurEigenvalues(TPtr: usize, n: i32, realPtr: usize, imagPtr: usize): void {
  let i: i32 = 0;
  while (i < n) {
    if (i === n - 1 || Math.abs(load<f64>(TPtr + ((<usize>((i + 1) * n + i)) << 3))) < 1e-14) {
 ...
 */
export function schurResidual(APtr: usize, QPtr: usize, TPtr: usize, n: i32, workPtr: usize): f64 {
  // Compute QT
  for (let i: i32 = 0; i < n; i++) {

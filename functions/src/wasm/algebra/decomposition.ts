 ...
 */
export function qrDecomposition(aPtr: usize, m: i32, n: i32, qPtr: usize): void {
 ...
 */
export function luSolve(luPtr: usize, n: i32, permPtr: usize, bPtr: usize, xPtr: usize): void {
 ...
 */
export function qrDecompositionSIMD(aPtr: usize, m: i32, n: i32, qPtr: usize): void {
  // Initialize Q as identity matrix
  for (let i: i32 = 0; i < m; i++) {
    for (let j: i32 = 0; j < m; j++) {
      store<f64>(qPtr + ((<usize>(i * m + j)) << 3), i === j ? 1.0 : 0.0);
 ...
 * @returns 0 if successful, 1 if not positive-definite
 */
export function choleskyDecompositionSIMD(aPtr: usize, n: i32, lPtr: usize): i32 {
  // Initialize L to zero
  for (let i: i32 = 0; i < n * n; i++) {
    store<f64>(lPtr + ((<usize>i) << 3), 0.0);
  }

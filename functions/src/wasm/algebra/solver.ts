...
export function lsolve(LPtr: usize, bPtr: usize, n: i32, xPtr: usize): void {
  for (let i: i32 = 0; i < n; i++) {
    ...
    for (let j: i32 = 0; j < i; j++) {
      sum -= load<f64>(LPtr + ((<usize>(i * n + j)) << 3)) * load<f64>(xPtr + ((<usize>j) << 3));
    }
...
export function usolve(UPtr: usize, bPtr: usize, n: i32, xPtr: usize): void {
  for (let i: i32 = n - 1; i >= 0; i--) {
    ...
    for (let j: i32 = i + 1; j < n; j++) {
      sum -= load<f64>(UPtr + ((<usize>(i * n + j)) << 3)) * load<f64>(xPtr + ((<usize>j) << 3));
    }
 ...
 */
export function lsolveMultiple(LPtr: usize, BPtr: usize, n: i32, m: i32, XPtr: usize): void {
 ...
 * @param XPtr - Pointer to solution matrix output (n x m, f64)
 */
export function usolveMultiple(UPtr: usize, BPtr: usize, n: i32, m: i32, XPtr: usize): void {
  for (let k: i32 = 0; k < m; k++) {
    // Solve for column k
    for (let i: i32 = n - 1; i >= 0; i--) {
      let sum: f64 = load<f64>(BPtr + ((<usize>(i * m + k)) << 3));
 ...
 * @param xPtr - Pointer to solution vector output (n, f64)
 */
export function lsolveBanded(LPtr: usize, bPtr: usize, n: i32, bandwidth: i32, xPtr: usize): void {
  for (let i: i32 = 0; i < n; i++) {
    let sum: f64 = load<f64>(bPtr + ((<usize>i) << 3));
 ...
 * @param xPtr - Pointer to solution vector output (n, f64)
 */
export function usolveBanded(UPtr: usize, bPtr: usize, n: i32, bandwidth: i32, xPtr: usize): void {
  for (let i: i32 = n - 1; i >= 0; i--) {
    let sum: f64 = load<f64>(bPtr + ((<usize>i) << 3));
...
export function solveTridiagonal(
  ...
  // Forward sweep
  store<f64>(cPrimePtr, load<f64>(cPtr) / load<f64>(bPtr));
  store<f64>(dPrimePtr, load<f64>(dPtr) / load<f64>(bPtr));

  for (let i: i32 = 1; i < n; i++) {
    const denom: f64 =
      load<f64>(bPtr + ((<usize>i) << 3)) -
      load<f64>(aPtr + ((<usize>i) << 3)) * load<f64>(cPrimePtr + ((<usize>(i - 1)) << 3));

    if (i < n - 1) {
      store<f64>(cPrimePtr + ((<usize>i) << 3), load<f64>(cPtr + ((<usize>i) << 3)) / denom);
    }
    store<f64>(
      dPrimePtr + ((<usize>i) << 3),
      (load<f64>(dPtr + ((<usize>i) << 3)) -
        load<f64>(aPtr + ((<usize>i) << 3)) * load<f64>(dPrimePtr + ((<usize>(i - 1)) << 3))) /
        denom
    );
  }
  ...
  for (let i: i32 = n - 2; i >= 0; i--) {
    store<f64>(
      ...
      load<f64>(dPrimePtr + ((<usize>i) << 3)) -
        load<f64>(cPrimePtr + ((<usize>i) << 3)) * load<f64>(xPtr + ((<usize>(i + 1)) << 3))
    );
  }
}
 ...
 */
export function lowerTriangularMV(LPtr: usize, xPtr: usize, n: i32, yPtr: usize): void {
  for (let i: i32 = 0; i < n; i++) {
    let sum: f64 = 0.0;
    for (let j: i32 = 0; j <= i; j++) {
      sum += load<f64>(LPtr + ((<usize>(i * n + j)) << 3)) * load<f64>(xPtr + ((<usize>j) << 3));
 ...
 * @param yPtr - Pointer to result vector output (n, f64)
 */
export function upperTriangularMV(UPtr: usize, xPtr: usize, n: i32, yPtr: usize): void {
  for (let i: i32 = 0; i < n; i++) {
    let sum: f64 = 0.0;
...
export function lsolveAll(
  ...
  for (let k: i32 = 0; k < n; k++) {
    if (load<i32>(isFreePtr + ((<usize>k) << 2)) === 1) {
      ...
      for (let i: i32 = 0; i < n; i++) {
        if (i === k) {
          store<f64>(solutionsPtr + ((<usize>(i * numCols + 1 + freeIdx)) << 3), 1.0);
        } else if (load<i32>(isFreePtr + ((<usize>i) << 2)) === 1) {
          store<f64>(solutionsPtr + ((<usize>(i * numCols + 1 + freeIdx)) << 3), 0.0);
        } else if (i < k) {
          let sum: f64 = 0.0;
          for (let j: i32 = 0; j < i; j++) {
            sum -=

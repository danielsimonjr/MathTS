...
export function median(dataPtr: usize, length: i32): f64 {
  ...
  if (length & 1) {
    ...
    // Even length
    return (
      (load<f64>(dataPtr + ((<usize>(mid - 1)) << 3)) + load<f64>(dataPtr + ((<usize>mid) << 3))) /
      2.0
    );
  }
}
...
export function quantile(dataPtr: usize, length: i32, p: f64): f64 {
  ...
  return (
    load<f64>(dataPtr + ((<usize>lower) << 3)) * (1 - fraction) +
    load<f64>(dataPtr + ((<usize>upper) << 3)) * fraction
  );
}
...
export function covariance(xPtr: usize, yPtr: usize, length: i32, ddof: i32): f64 {
  ...
  for (let i: i32 = 0; i < length; i++) {
    sumProd +=
      (load<f64>(xPtr + ((<usize>i) << 3)) - meanX) * (load<f64>(yPtr + ((<usize>i) << 3)) - meanY);
  }
...
export function movingAverage(dataPtr: usize, length: i32, window: i32, resultPtr: usize): i32 {
  ...
  for (let i: i32 = window; i < length; i++) {
    windowSum +=
      load<f64>(dataPtr + ((<usize>i) << 3)) - load<f64>(dataPtr + ((<usize>(i - window)) << 3));

 ...
 * @param resultPtr - Pointer to 3x3 rotation matrix (f64, 9 elements, row-major)
 */
export function rotationMatrixAxisAngle(axisPtr: usize, angle: f64, resultPtr: usize): void {
  const c: f64 = Math.cos(angle);
  const s: f64 = Math.sin(angle);
  const t: f64 = 1.0 - c;
 ...
 * @param resultPtr - Pointer to 3x3 rotation matrix (f64, 9 elements, row-major)
 */
export function rotationMatrixEulerZYX(yaw: f64, pitch: f64, roll: f64, resultPtr: usize): void {
 ...
 * @param resultPtr - Pointer to 3x3 rotation matrix (f64, 9 elements, row-major)
 */
export function rotationMatrixEulerXYZ(alpha: f64, beta: f64, gamma: f64, resultPtr: usize): void {
 ...
 */
export function quaternionMultiply(q1Ptr: usize, q2Ptr: usize, resultPtr: usize): void {
 ...
 */
export function quaternionSlerp(q1Ptr: usize, q2Ptr: usize, t: f64, resultPtr: usize): void {
  const q1w: f64 = load<f64>(q1Ptr);
  const q1x: f64 = load<f64>(q1Ptr + 8);
  const q1y: f64 = load<f64>(q1Ptr + 16);
  const q1z: f64 = load<f64>(q1Ptr + 24);
 ...
 */
export function quaternionFromAxisAngle(axisPtr: usize, angle: f64, resultPtr: usize): void {
 ...
 * @param resultPtr - Pointer to rotated point [x', y', z'] (f64, 3 elements)
 */
export function rotateByMatrix(pointPtr: usize, RPtr: usize, resultPtr: usize): void {
  ...
  store<f64>(
    resultPtr + 8,
    load<f64>(RPtr + 24) * px + load<f64>(RPtr + 32) * py + load<f64>(RPtr + 40) * pz
  );
  store<f64>(
    resultPtr + 16,
    load<f64>(RPtr + 48) * px + load<f64>(RPtr + 56) * py + load<f64>(RPtr + 64) * pz
  );
}
...
export function isRotationMatrix(RPtr: usize, tol: f64): bool {
  ...
  const det: f64 = R0 * (R4 * R8 - R5 * R7) - R1 * (R3 * R8 - R5 * R6) + R2 * (R3 * R7 - R4 * R6);

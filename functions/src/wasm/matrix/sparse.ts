  ...
  while (tail > 0) {
    ...
    while (k < pEnd) {
      ...
      if (load<i32>(donePtr + ((<usize>neighbor) << 2)) === 0) {
        ...
        store<i32>(
          stackAdjPtr + ((<usize>tail) << 2),
          load<i32>(gPtrPtr + ((<usize>neighbor) << 2))
        );
        tail++;
        found = true;
        break;
  ...
  while (depth >= 0) {
    ...
    for (let kk: i32 = pos; kk < pEnd; kk++) {
      ...
      if (load<i32>(markPtr + ((<usize>child) << 2)) === 0) {
        ...
        store<i32>(
          stackPosPtr + ((<usize>depth) << 2),
          pc >= 0 ? load<i32>(gPtrPtr + ((<usize>pc) << 2)) : 0
        );
        foundChild = true;
        break;
      }
 ...
 */
export function csPost(parentPtr: usize, n: i32, postPtr: usize, workPtr: usize): void {
  // Work area layout:
  // [0, n): firstChild
  // [n, 2n): nextSibling
  // [2n, 3n): stack
  ...
  for (let k: i32 = 0; k < n; k++) {
    ...
    for (let p: i32 = top; p < n; p++) {
      ...
      const lki: f64 =
        ...
        load<f64>(lValuesPtr + load<i32>(lPtrPtr + ((<usize>i) << 2)) * 8);
...
export function csLu(
  ...
  for (let k: i32 = 0; k < n; k++) {
    ...
    const top: i32 = csSpsolvePattern(
      aIndexPtr,
      aPtrPtr,
      lIndexPtr,
      lPtrPtr,
    ...
    for (let p: i32 = top; p < n; p++) {
      ...
      // x[j] = x[j] / L[j,j]
      const ljj: f64 = load<f64>(
        lValuesPtr + ((<usize>load<i32>(lPtrPtr + ((<usize>jnew) << 2))) << 3)
      );
      if (ljj === 0.0) continue;
    ...
    while (depth >= 0) {
      ...
      for (let q: i32 = pos; q < pEnd; q++) {
        ...
        if (load<i32>(markPtr + ((<usize>child) << 2)) !== currentMark) {
          ...
          store<i32>(
            stackPosPtr + ((<usize>depth) << 2),
            pc >= 0 ? load<i32>(lPtrPtr + ((<usize>pc) << 2)) : 0
          );
          foundChild = true;
          break;
        }
    ...
    for (let j: i32 = 0; j < k; j++) {
      ...
      for (let p: i32 = vStart; p < vEnd; p++) {
        ...
        store<f64>(
          xPtr + ((<usize>i) << 3),
          load<f64>(xPtr + ((<usize>i) << 3)) -
            betaJ * load<f64>(vValuesPtr + ((<usize>p) << 3)) * vTx
        );
      }
    }
  ...
  if (transpose) {
    ...
    for (let k: i32 = n - 1; k >= 0; k--) {
      ...
      for (let p: i32 = vStart; p < vEnd; p++) {
        ...
        store<f64>(
          yPtr + ((<usize>i) << 3),
          load<f64>(yPtr + ((<usize>i) << 3)) -
            betaK * load<f64>(vValuesPtr + ((<usize>p) << 3)) * vTy
        );
      }
    }
  } else {
...
export function csAmd(
  ...
  for (let j: i32 = 0; j < n; j++) {
    const count: i32 =
      load<i32>(aPtrPtr + ((<usize>(j + 1)) << 2)) - load<i32>(aPtrPtr + ((<usize>j) << 2));
  ...
  for (let k: i32 = 0; k < n; k++) {
    ...
    for (let j: i32 = 0; j < n; j++) {
      if (
        load<i32>(eliminatedPtr + ((<usize>j) << 2)) === 0 &&
        load<i32>(degreePtr + ((<usize>j) << 2)) < minDeg
      ) {
...
export function csRcm(
  ...
  for (let s: i32 = 0; s < n; s++) {
    ...
    for (let j: i32 = s; j < n; j++) {
      if (
        load<i32>(visitedPtr + ((<usize>j) << 2)) === 0 &&
        load<i32>(degreePtr + ((<usize>j) << 2)) < minDeg
      ) {
        minDeg = load<i32>(degreePtr + ((<usize>j) << 2));
        cStart = j;
    ...
    while (head < tail) {
      ...
      for (let i: i32 = 1; i < numNeighbors; i++) {
        ...
        while (
          j >= 0 &&
          load<i32>(degreePtr + load<i32>(neighborsPtr + ((<usize>j) << 2)) * 4) > keyDeg
        ) {
          store<i32>(
            neighborsPtr + ((<usize>(j + 1)) << 2),
            load<i32>(neighborsPtr + ((<usize>j) << 2))
          );
          j--;
        }
    ...
    for (let p: i32 = bStart; p < bEnd; p++) {
      ...
      for (let q: i32 = aStart; q < aEnd; q++) {
        ...
        if (load<i32>(wPtr + ((<usize>i) << 2)) < mark) {
          ...
          store<f64>(
            xPtr + ((<usize>i) << 3),
            load<f64>(xPtr + ((<usize>i) << 3)) + load<f64>(aValuesPtr + ((<usize>q) << 3)) * bkj
          );
        }
      }
    }
...
export function csMultNnzEstimate(
  ...
  for (let j: i32 = 0; j < bCols; j++) {
    ...
    for (let p: i32 = bStart; p < bEnd; p++) {
      ...
      if (k < aCols) {
        colNnz +=
          load<i32>(aPtrPtr + ((<usize>(k + 1)) << 2)) - load<i32>(aPtrPtr + ((<usize>k) << 2));
      }
    }

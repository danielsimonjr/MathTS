 ...
 */
export function balanceMatrix(matrixPtr: usize, n: i32, tolerance: f64, transformPtr: usize): i32 {
  ...
  for (let i: i32 = 0; i < n - 2; i++) {
    ...
    for (let j: i32 = i + 2; j < n; j++) {
      ...
      for (let k: i32 = 0; k < n; k++) {
        ...
        store<f64>(
          matrixPtr + jkIdx,
          load<f64>(matrixPtr + jkIdx) - factor * load<f64>(matrixPtr + ip1kIdx)
        );
      }
      ...
      for (let k: i32 = 0; k < n; k++) {
        ...
        store<f64>(
          matrixPtr + kIp1Idx,
          load<f64>(matrixPtr + kIp1Idx) + factor * load<f64>(matrixPtr + kjIdx)
        );
      }
      ...
      if (computeTransform) {
        for (let k: i32 = 0; k < n; k++) {
          ...
          store<f64>(
            transformPtr + jkIdx,
            load<f64>(transformPtr + jkIdx) - factor * load<f64>(transformPtr + ip1kIdx)
          );
        }
      }
    }
 ...
 */
export function eigenvalues2x2(a: f64, b: f64, c: f64, d: f64, eigenvaluesPtr: usize): void {
...
export function qrAlgorithm(
  ...
  while (currentN > 0 && iterSinceDeflation < maxIterations) {
    ...
    if (currentN === 1) {
      ...
      store<f64>(
        eigenvaluesRealPtr + ((<usize>numEigenvalues) << 3),
      ...
      const diagN1: f64 = Math.abs(
        load<f64>(workMatrix + ((<usize>((currentN - 1) * n + (currentN - 1))) << 3))
      );
      const diagN2: f64 = Math.abs(
        load<f64>(workMatrix + ((<usize>((currentN - 2) * n + (currentN - 2))) << 3))
      );
      ...
      if (subdiag < tolerance * (diagN1 + diagN2 + 1e-15)) {
        ...
        const diagN3: f64 =
          currentN > 2
            ? Math.abs(
                load<f64>(workMatrix + ((<usize>((currentN - 3) * n + (currentN - 3))) << 3))
              )
            : 0.0;

        if (currentN === 2 || subdiag2 < tolerance * (diagN2 + diagN3 + 1e-15)) {
          // Extract 2x2 block eigenvalues
          const a: f64 = load<f64>(
            workMatrix + ((<usize>((currentN - 2) * n + (currentN - 2))) << 3)
          );
          const b: f64 = load<f64>(
            workMatrix + ((<usize>((currentN - 2) * n + (currentN - 1))) << 3)
          );
          const c: f64 = load<f64>(
            workMatrix + ((<usize>((currentN - 1) * n + (currentN - 2))) << 3)
          );
          const d: f64 = load<f64>(
            workMatrix + ((<usize>((currentN - 1) * n + (currentN - 1))) << 3)
          );
 ...
 */
function sortComplexEigenvalues(realPtr: usize, imagPtr: usize, n: i32): void {
 ...
 * @param fullN - Full matrix dimension
 */
export function hessenbergQRStep(matrixPtr: usize, n: i32, fullN: i32): void {
  if (n < 2) return;

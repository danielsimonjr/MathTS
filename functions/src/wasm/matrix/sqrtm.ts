...
export function sqrtm(
  ...
  for (let iter: i32 = 0; iter < maxIterations; iter++) {
    ...
    let maxDiff: f64 = 0.0;
    for (let i: i32 = 0; i < nn; i++) {
      const diff: f64 = Math.abs(
        load<f64>(YPtr + ((<usize>i) << 3)) - load<f64>(YkPtr + ((<usize>i) << 3))
      );
      if (diff > maxDiff) {
        maxDiff = diff;
      }
    ...
    for (let row: i32 = 0; row < n; row++) {
      if (row !== col) {
        ...
        for (let j: i32 = 0; j < n; j++) {
          ...
          store<f64>(
            invPtr + rowIdx,
            load<f64>(invPtr + rowIdx) - factor * load<f64>(invPtr + colIdx)
          );
        }
      }
    }
  ...
  for (let iter: i32 = 0; iter < maxIterations; iter++) {
    ...
    for (let i: i32 = 0; i < n; i++) {
      for (let j: i32 = 0; j < n; j++) {
        ...
        for (let k: i32 = 0; k < n; k++) {
          sum +=
            load<f64>(tempPtr + ((<usize>(i * n + k)) << 3)) *
 ...
 */
export function sqrtmCholesky(matrixPtr: usize, n: i32, resultPtr: usize, workPtr: usize): i32 {

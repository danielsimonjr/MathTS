 ...
 */
function transposeInternal(aPtr: usize, rows: i32, cols: i32, resultPtr: usize): void {
 ...
 */
function matAddInternal(aPtr: usize, bPtr: usize, size: i32, resultPtr: usize): void {
 ...
 * @param resultPtr - Pointer to result matrix
 */
function matSubInternal(aPtr: usize, bPtr: usize, size: i32, resultPtr: usize): void {
  for (let i: i32 = 0; i < size; i++) {
    const offset: usize = (<usize>i) << 3;
    store<f64>(resultPtr + offset, load<f64>(aPtr + offset) - load<f64>(bPtr + offset));
  }
 ...
 * @param resultPtr - Pointer to result matrix
 */
function scalarMulInternal(aPtr: usize, s: f64, size: i32, resultPtr: usize): void {
  for (let i: i32 = 0; i < size; i++) {
    const offset: usize = (<usize>i) << 3;
 ...
 */
function invInternal(aPtr: usize, n: i32, resultPtr: usize, augPtr: usize): i32 {
  const width: i32 = 2 * n;
 ...
 */
export function pinv(aPtr: usize, rows: i32, cols: i32, resultPtr: usize, workPtr: usize): i32 {
 ...
 */
export function sqrtmSPD(aPtr: usize, n: i32, resultPtr: usize, workPtr: usize): i32 {
 ...
 */
export function expm(aPtr: usize, n: i32, resultPtr: usize, workPtr: usize): i32 {
  const n2: i32 = n * n;
  const n2Bytes: usize = (<usize>n2) << 3;
...
export function eigsSymmetric(
  ...
  for (let iter: i32 = 0; iter < maxIter * n; iter++) {
    ...
    store<f64>(
      workPtr + ((<usize>(p * n + p)) << 3),
      c * c * app - 2.0 * c * s * apq + s * s * aqq
    );
    store<f64>(
      workPtr + ((<usize>(q * n + q)) << 3),
  ...
  for (let i: i32 = 0; i < n; i++) {
    store<f64>(
      ...
      load<f64>(workPtr + ((<usize>(i * n + i)) << 3))
    );
  }
...
export function eigs(
  ...
  for (let k: i32 = 0; k < n - 2; k++) {
    ...
    for (let j: i32 = k; j < n; j++) {
      ...
      for (let i: i32 = 0; i < uLen; i++) {
        dot +=
          load<f64>(uPtr + ((<usize>i) << 3)) *
          load<f64>(HPtr + ((<usize>((k + 1 + i) * n + j)) << 3));
      }
      dot *= 2.0;
      for (let i: i32 = 0; i < uLen; i++) {
    ...
    for (let i: i32 = 0; i < n; i++) {
      ...
      for (let j: i32 = 0; j < uLen; j++) {
        dot +=
          load<f64>(HPtr + ((<usize>(i * n + (k + 1 + j))) << 3)) *
          load<f64>(uPtr + ((<usize>j) << 3));
      }
      dot *= 2.0;
      for (let j: i32 = 0; j < uLen; j++) {
  ...
  for (let iter: i32 = 0; iter < maxIter && m > 1; iter++) {
    ...
    for (let i: i32 = m - 1; i > 0; i--) {
      ...
      if (subdiag < tol * (diag1 + diag2)) {
        ...
        if (i === m - 1) {
          store<f64>(
            eigenvaluesPtr + ((<usize>(m - 1)) << 3),
            load<f64>(HPtr + ((<usize>((m - 1) * n + (m - 1))) << 3))
          );
          m--;
          deflate = true;
          break;
 ...
 */
export function spectralRadius(aPtr: usize, n: i32, workPtr: usize): f64 {

...
export function erf(x: f64): f64 {
  ...
  const y: f64 =
    1.0 - (ERF_A1 * t + ERF_A2 * t2 + ERF_A3 * t3 + ERF_A4 * t4 + ERF_A5 * t5) * Math.exp(-x * x);
...
export function zeta(s: f64): f64 {
  ...
  const factor: f64 =
    Math.pow(2.0, s) * Math.pow(PI, s - 1.0) * Math.sin((PI * s) / 2.0) * gamma(1.0 - s);
...
export function besselJ1(x: f64): f64 {
  ...
  if (x < 8.0) {
    ...
    return sign * Math.sqrt(0.636619772 / x) * (Math.cos(xx) * ans1 - z * Math.sin(xx) * ans2);
  }
}
...
export function besselY1(x: f64): f64 {
  ...
  if (x < 8.0) {
    ...
    const ans2: f64 =
      ...
      y *
        (424441966400.0 +
          y * (3733650367.0 + y * (22459040.02 + y * (102042.605 + y * (354.9632885 + y * 1.0)))));

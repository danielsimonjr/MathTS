 ...
 */
export function fft2d(dataPtr: usize, rows: i32, cols: i32, inverse: i32, workPtr: usize): void {
  // FFT on rows
...
export function convolveSIMD(
  ...
  for (let i: i32 = 0; i < size; i++) {
    ...
    v128.store(
      resultPtr + idx,
      f64x2.replace_lane(f64x2.replace_lane(f64x2.splat(0.0), 0, resReal), 1, resImag)
    );
  }

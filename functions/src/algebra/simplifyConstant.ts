    ...
    function _exactFraction(n: any, options: any): any {
      ...
      if (exactFractions && isBounded(n) && fraction) {
        ...
        const fractionsLimit =
          options && typeof options.fractionsLimit === 'number'
      ...
      switch (node.type) {
        ...
        case 'OperatorNode': {
          ...
          if (isOperatorNode(node) && node.isUnary()) {
            ...
            args = (node as OperatorNode).args.map((arg: MathNode) =>
              foldFraction(arg, options)
            )
            res = foldOp(fn, args, makeNode, options)
          }
        ...
        case 'IndexNode': {
          return new IndexNode(
            (node as IndexNode).dimensions.map((n: MathNode) =>
        ...
        case 'ObjectNode': {
          ...
          for (const prop in (node as ObjectNode).properties) {
            foldProps[prop] = simplifyConstant(
              (node as ObjectNode).properties[prop],
              options
            )
          }

    ...
    function _simplifyCore(
      ...
      if (isFunctionNode(node)) {
        ...
        if (op) {
          ...
          if (
            node.args.length > 2 &&
            hasProperty(node, 'associative', context)
          ) {
          ...
          return new FunctionNode(
            _simplifyCore((node as any).fn as any),
            node.args.map((n: MathNode) => _simplifyCore(n, options))
          )
        }
      }
      ...
      if (isOperatorNode(node)) {
        return new OperatorNode(
          node.op,
          (node as any).fn,
          node.args.map((a: MathNode) => _simplifyCore(a, options))
        )
      }
      if (isArrayNode(node)) {
        return new ArrayNode(
          node.items.map((n: MathNode) => _simplifyCore(n, options))
        )
      }

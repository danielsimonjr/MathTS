...
export function importFactory(
  ...
  function _wrap(fn: FunctionWithSignature): FunctionWithSignature {
    const wrapper = function wrapper(this: unknown, ...fnArgs: unknown[]): unknown {
      const args: unknown[] = [];
  ...
  function _importFactory(
    ...
    if (name.includes('.')) {
      throw new Error(
        'Factory name should not contain a nested path. ' + 'Name: ' + JSON.stringify(name)
      );
    }

    const namespace: Record<string, unknown> = isTransformFunctionFactory(factory)

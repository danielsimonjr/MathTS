...
export const createIndexNode = /* #__PURE__ */ factory(
  name,
  dependencies,
  ({ Node, size }: Dependencies) => {
    class IndexNode extends Node {
      ...
      _compile(
        ...
        return function evalIndexNode(
          ...
          const dimensions = map(
            evalDimensions,
            function (evalDimension: CompileFunction): any {
       ...
       */
      forEach(
        callback: (child: Node, path: string, parent: IndexNode) => void
      ): void {
       ...
       */
      map(
        callback: (child: Node, path: string, parent: IndexNode) => Node
      ): IndexNode {
      ...
      _toHTML(_options?: StringOptions): string {
        ...
        if (this.dotNotation) {
          return (
            ...
            escape(this.getObjectProperty() as string) +
            '</span>'
          )
        } else {
          return (

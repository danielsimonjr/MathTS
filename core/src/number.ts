  ...
  switch (notation) {
    ...
    case 'auto':
      ...
      return toPrecision(value, precision, options as FormatOptions).replace(
        /((\.\d*?)(0+))($|e)/,
        function (_match: string, _all: string, digits: string, _zeros: string, e: string) {
 ...
 */
export function normalizeFormatOptions(options?: FormatOptions | number): NormalizedFormatOptions {
...
export function toEngineering(value: number | string, precision?: number): string {
  ...
  const decimalVal =
    (isNumber(precision) && decimals.length) || decimals.match(/[1-9]/) ? '.' + decimals : '';

  const str =
    c.slice(0, decimalIdx).join('') + decimalVal + 'e' + (e >= 0 ? '+' : '') + newExp.toString();
...
export function toExponential(value: number | string, precision?: number): string {
  ...
  const first = c.shift()!;
  return (
    rounded.sign + first + (c.length > 0 ? '.' + c.join('') : '') + 'e' + (e >= 0 ? '+' : '') + e
  );
}

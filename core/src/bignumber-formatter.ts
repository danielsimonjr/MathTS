/**
 * BigNumber (decimal.js-shaped) formatting — canonical copy.
 *
 * Consolidated onto core (Bucket B, slice 1): `expression/src/utils/bignumber/formatter.ts`
 * and `functions/src/utils/bignumber/formatter.ts` carried byte-identical bodies (mathjs-derived,
 * dead `.ts→.ts` sync residue). This is the ONE implementation; both packages re-export it via
 * `@danielsimonjr/mathts-core/internal` (see `number.ts`/`object.ts` and the
 * `project-all-libraries-build-on-core` principle). Proven equivalent to both prior copies via a
 * fast-check property harness (`core/tests/helpers/equivalence.ts`) before the redirect —
 * see `functions/tests/dedup-bucketB-equivalence.test.ts`.
 *
 * NOTE: exported under aliased names from `internal.ts` (`formatBigNumber`, `toEngineeringBigNumber`,
 * `toExponentialBigNumber`, `toFixedBigNumber`) — `./number.ts` already publicly exports `format`,
 * `toEngineering`, `toExponential`, and `toFixed` for PLAIN numbers, and `export *` from both modules
 * in the same barrel would collide (TS2308). Each package's `utils/bignumber/formatter.ts` shim
 * re-imports the aliased names and re-exports them under the original local names, so no consumer
 * of either package needs to change anything.
 */
